package com.jwh.gwt.design.shared.html;
...
public class TagUtil {
	...
	/**
	 * Assure that the specified sequence of tags is available. If they do not exist, create them.
	 * @param parent
	 * @param parents
	 * @return an element with the specified heritage
	 */
	public static Element assureParents(final Element parent,
			final Tag... parents) {
	 ...
	 * @param depth
	 * @return
	 * @throws NotFound if no element with the specified tag is found within the specified depth
	 */
	public static Element assureParent(final Element element, final Tag tag,
			int depth) throws NotFound {
		if (element == null || depth < 0) {
			throw NotFound.getInstance();
		}
		depth--;
		if (!element.getTagName().equalsIgnoreCase(tag.name())) {
			return assureParent((Element) element.getParentElement(), tag,
					depth);
		}
		return element;
	}
	public static void registerListener(Element element, EventListener listener, boolean includeChildren) {
